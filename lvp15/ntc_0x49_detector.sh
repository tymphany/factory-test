...
i2ctransfer -y -f 3 w3@0x49 0x03 0x05 0x00
i2ctransfer -y -f 3 w1@0x49 0x01 r2
sleep 1s

GPIO_STATUS=`cat /sys/class/gpio/gpio47/value`
if [ ${GPIO_STATUS} -eq 0 ]; then
        echo "[NTC 0x49 Detect]: OK"
fi

i2ctransfer -y -f 3 w3@0x49 0x03 0x7f 0x00
i2ctransfer -y -f 3 w1@0x49 0x01 r2
sleep 1s

GPIO_STATUS=`cat /sys/class/gpio/gpio47/value`
#if [ ${GPIO_STATUS} -eq 1 ]; then
#        echo "[NTC 0x49 Detect]: Over"
#fi
